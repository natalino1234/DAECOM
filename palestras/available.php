...
if (!isset($_REQUEST["btn_buscar"])) {
    ...
    $sql = "select p.pal_id as pal_id, p.pal_autorizacao as pal_autorizacao, p.alu_organizacao as pal_organizacao, p.pal_nome as pal_nome, "
            ...
            . "from tb_Palestra as p left join tb_Pal_Alu_Participar as t on (p.pal_id = t.pal_id) Left Join tb_Aluno as a on (a.alu_matricula = p.pal_palestrante)  "
            . "where p.pal_autorizacao = 1 AND pal_" . $_REQUEST["busca"] . " like '%" . $_REQUEST["campo_busca"] . "%' and p.alu_organizacao <> '" . $_SESSION["matricula"] . "' "
            . "order by p.pal_id DESC LIMIT $idpag,$quantidadelinhas";
    $sqlcountregs = "Select count(q.pal_id) as qtdregs "
            . "from (select p.pal_id "
            . "from tb_Palestra as p left join tb_Pal_Alu_Participar as t on (p.pal_id = t.pal_id) Left Join tb_Aluno as a on (a.alu_matricula = p.pal_palestrante)  "
            . "where p.pal_autorizacao = 1 AND pal_" . $_REQUEST["busca"] . " like '%" . $_REQUEST["campo_busca"] . "%'"
            . " and p.alu_organizacao <> '" . $_SESSION["matricula"] . "' "
            . "group by p.pal_id) as q";
}
if ($search === "") {
    $search = "?";
...
<html>
    ...
    <body>
        ...
        <div id='page-container'>
            ...
            <div id='content'>
                <div id="title">Palestras Disponíveis</div>
                <form id='search' class="one-line" method='get' action="/Palestras/Disponiveis">
                    <div class="input">
                ...
                <table class="list">
                    <tr>
                        ...
                        <tr>
                            <td colspan="5">
                                Não temos Palestras Disponíveis.
                            </td>
                        </tr>
                    <?php } ?>
