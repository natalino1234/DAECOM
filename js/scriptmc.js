$(document).ready(function () {
    ...
    $("a.edits").click(function () {
        $.ajax({type: "POST", url: "/functions/alterarcadastro.php", data: {"colAlt": "senha"}})
                .done(function (data) {
                    if (data === "1") {
                        alertUser("Foi enviado um email para alteração da senha.");
                    } else {
                        alertUser("Houve um erro na solicitação de alteração, tente novamente.");
                    }
                });
    });
});
function alteraReg(input, a) {
    ...
    $.ajax({type: "POST", url: "/functions/alterarcadastro.php", data: {"colAlt": "alu_" + name, "valueAlt": value + ""}})
            .done(function (data) {
                ...
                if (data === "1") {
                    alertUser("Alteração realizada com sucesso.");
                    a.css({
                        "color": "#B5684D",
                        "border-bottom": "1px dotted #B5684D"
                    });
...
function alteraReg(input) {
    var value = input.value;
    var name = input.id;
    $.ajax({type: "POST", url: "/functions/alterarcadastro.php", data: {"colAlt": "" + name, "valueAlt": value + ""}})
            .done(function (data) {
                if (data === "1") {
                    alertUser("Alteração realizada com sucesso.");
                } else {
                    input.value = document.getElementById(input.id+"b").value;
                    alertUser("Houve um erro na solicitação de alteração, tente novamente.");
                }
            });
}
