...
$sql = ""
        ...
        . "as h on (h.min_id = p.min_id) left join tb_Min_Alu_Presenca as pres on (pres.min_id = p.min_id and pres.alu_matricula like '" . $_SESSION["matricula"] . "') "
        . "where ((select MAX(mih_dia) from tb_Min_Horario where min_id = p.min_id) < NOW() and p.min_autorizacao = 1) "
        . "group by p.min_id "
...
$sqlcountregs = "Select count(q.min_id) as qtdregs "
        ...
        . "left join tb_Min_Alu_Participar as t on (p.min_id = t.min_id) "
        . "left join tb_Min_Alu_Presenca as pres on (pres.min_id = p.min_id AND pres.alu_matricula = " . $_SESSION["matricula"] . ") "
        . "where (select MAX(mih_dia) from tb_Min_Horario where min_id = p.min_id) < NOW() and p.min_autorizacao = 1 "
...
if (isset($_REQUEST["btn_buscar"])) {
    if ($_REQUEST["btn_buscar"]) {
        ...
        $sql = "select p.min_id as min_id, p.min_autorizacao as min_autorizacao, p.alu_organizacao as min_organizacao, p.min_nome as min_nome, "
                ...
                . "tb_Aluno as a on (a.alu_matricula = p.min_professor) inner join tb_Min_Horario as h on (h.min_id = p.min_id)"
                . "where (select MAX(mih_dia) from tb_Min_Horario where min_id = p.min_id) < NOW() and p.min_autorizacao = 1 "
                . "and pres.alu_matricula = '" . $_SESSION["matricula"] . "' or p.alu_organizacao = '" . $_SESSION["matricula"] . "' "
                . "min_" . $_REQUEST["busca"] . " like '%" . $_REQUEST["campo_busca"] . "%' OR "
                . "min_professor like '%" . $_REQUEST["campo_busca"] . "%' "
                . "order by p.min_id DESC LIMIT $idpag,$quantidadelinhas";
        $sqlcountregs = "Select count(q.min_id) as qtdregs "
                . "from (select p.min_id "
                . "from tb_Minicurso as p left join tb_Min_Alu_Participar as t on (p.min_id = t.min_id) inner join "
                . "tb_Aluno as a on (a.alu_matricula = p.min_professor) "
                . "left join tb_Min_Alu_Presenca as pres on (pres.min_id = p.min_id) "
                . "where min_" . $_REQUEST["busca"] . " like '%" . $_REQUEST["campo_busca"] . "%' OR "
                . "alu_nome like '%" . $_REQUEST["campo_busca"] . "%' "
                . "and (pres.alu_matricula = '" . $_SESSION["matricula"] . "' or p.alu_organizacao = '" . $_SESSION["matricula"] . "') "
...
<html>
    ...
    <body>
        ...
        <div id='page-container'>
            ...
            <div id='content'>
                <div id="title">Todos os Minicursos</div>
                <div id="properties">
                    <?php
